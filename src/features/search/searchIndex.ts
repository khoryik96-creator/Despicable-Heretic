...
export const searchIndex: SearchableWikiItem[] = [
  ...chapters.map((item) => ({ id: item.id, title: item.title, subtitle: `Season ${item.season} · Chapter ${item.number}`, section: 'chapters' as const, body: item.summary, tags: item.tags })),
  ...
  ...techniques.map((item) => ({ id: item.id, title: item.name, subtitle: `${item.user} · ${item.tier}`, section: 'techniques' as const, body: item.description, tags: [item.user, item.category, item.tier] })),
